<?php
    ...
    $ipFilters = isset($config['modules']['gii']['ipFilters']) && is_array($config['modules']['gii']['ipFilters'])
        ? $config['modules']['gii']['ipFilters']
        : array('127.0.0.1', '::1');
    if(in_array($_SERVER['REMOTE_ADDR'], $ipFilters) && isset($_GET['debug'])) {
        $config = CMap::mergeArray($config, array(
            'components' => array(
                'log' => array(
                    'routes' => array(
                        array('class' => 'system.logging.CWebLogRoute'),
                        array('class' => 'system.logging.CProfileLogRoute'),
                    ),
                ),
            ),
        ));
